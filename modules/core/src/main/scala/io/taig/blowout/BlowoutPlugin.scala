...
// `sbt.io` would shadow the root `io` package that this plugin lives in
import sbt.{io as _, *}
...
object BlowoutPlugin extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[?]] = Def.settings(
    ...
    // sbt 2 caches tasks by default, which would skip the file system effects these tasks exist for
    blowoutGenerate := Def.uncached {
      blowoutGenerators.value.map { generator =>
        IO.write(generator.target, blowoutRender.value(generator))
        generator.target
      }
    },
    blowoutCheck := Def.uncached {
